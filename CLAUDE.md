...
**One call:** `diagnose_dag_run(dag_id="the_dag")` — this automatically finds the latest failed run, shows all task states, and fetches logs for failed tasks.
...
All tools return human-readable error messages (never raw HTTP errors). If you see "Error in <tool>:", it means the Airflow API returned an error — the message includes the HTTP status and reason.
